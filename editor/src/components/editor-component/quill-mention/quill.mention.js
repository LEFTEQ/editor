...
export default class Mention {
  constructor(quill, options) {
    ...
    this.options.container.appendChild(this.mentionContainer);
  ...
  selectItem() {
    ...
    this.quill.setSelection(this.mentionCharPos + 1, Quill.sources.API);
    this.hideMentionList();
  }
  ...
  onSomethingChange() {
    ...
    if (mentionCharIndex > -1) {
      ...
      const textAfter = beforeCursorPos.substring(mentionCharIndex + 3);
      console.log("textafter", textAfter);

      if (textAfter.length >= this.options.minChars && this.hasValidChars(textAfter)) {
        const mentionChar = beforeCursorPos.substring(mentionCharIndex, mentionCharIndex + 3);
